...
int partition(int array[], int low, int high) {
    ...
    for (int j = low; j < high; j++) {
        if (array[j] <= pivot) {
            i++;
            rotacionar(&array[i], &array[j]);
        }
    }

    rotacionar(&array[i + 1], &array[high]);
...
void executeQuickSort() {
    ...
    int quickSortArr[] = {4, 10, 3, 5, 1};
    int iteracoes = 0;
    int N = sizeof(quickSortArr)/sizeof(*quickSortArr);
    ...
    printf("Levou %d segundos e %d millisegundos (%d clocks) para executar com QuickSort\n",
           msec/1000, msec%1000, iteracoes);
